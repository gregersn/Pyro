...
TEST_CASE("Graphics objects can be initialized", "[graphics]") {
    SECTION("Create a graphics object") {
        Pyro::Graphics *graphics = Pyro::Graphics::create(400, 300, Pyro::GraphicsMode::P2D);
...
TEST_CASE("Graphics objects can set and get pixels", "[graphics]") {
    SECTION("Set and get") {
        Pyro::Graphics *pg = Pyro::Graphics::create(2, 2, Pyro::GraphicsMode::P2D);
        pg->set(0, 0, 0xff000000);
        pg->set(0, 1, 0xffff0000);
        pg->set(1, 0, 0xff00ff00);
        pg->set(1, 1, 0xff0000ff);

        REQUIRE(pg->get(0, 0) == 0xff000000);
        REQUIRE(pg->get(0, 1) == 0xffff0000);
        REQUIRE(pg->get(1, 0) == 0xff00ff00);
        REQUIRE(pg->get(1, 1) == 0xff0000ff);

    }
}

TEST_CASE("Graphics can have backgrounds", "[graphics]") {
    Pyro::Graphics *pg = Pyro::Graphics::create(4, 4, Pyro::GraphicsMode::P2D);
...
TEST_CASE("Fill can be set", "[graphics]") {
    Pyro::Graphics *pg = Pyro::Graphics::create(8, 8, Pyro::GraphicsMode::P2D);
...
TEST_CASE("Stroke can be set", "[graphics]") {
    Pyro::Graphics *pg = Pyro::Graphics::create(10, 1, Pyro::GraphicsMode::P2D);
...
TEST_CASE("Shapes can be drawn", "[graphics]") {
    ...
    SECTION("Draw rect") {
        ...
        SECTION("RectMode center") {
            ...
            remove("rectmode_center.png");
        }
    }
}

TEST_CASE("Select different color modes") {
    Pyro::Graphics *pg = Pyro::Graphics::create(3, 3, Pyro::GraphicsMode::P2D);
