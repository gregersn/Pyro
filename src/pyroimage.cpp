...
namespace Pyro
{
    ...
    void Image::init(int width, int height, int format, int factor) {
        this->dpi = 72;
        this->density = factor;
        this->_width = width;
        this->_height = height;
    ...
    void Image::savePNG(const std::string &filename, unsigned int dpi) {
        ...
        switch(this->format) {
            case GRAY:
                ...
                for(int i = 0; i < this->_pixelwidth * this->_pixelheight; i++) {
    ...
    void *Image::get_pre_multiplied_data() {
        if(this->cache == nullptr) {
            this->cache = malloc(this->_width * this->_height * sizeof(unsigned char) * this->format);
    ...
    void Image::update_pixels(int x, int y, int w, int h) {
        ...
        if(!modified) {
            mx1 = max(0, x);
            mx2 = min(_pixelwidth, x2);
            my1 = max(0, y);
            my2 = min(_pixelheight, y2);
            modified = true;
        } else {
            if (x < mx1) mx1 = max(0, x);
            if (x > mx2) mx2 = min(_pixelwidth, x);
            if (y < my1) my1 = max(0, y);
            if (y > my2) my2 = min(_pixelheight, y);

            if (x2 < mx1) mx1 = max(0, x2);
            if (x2 > mx2) mx2 = min(_pixelwidth, x2);
            if (y2 < my1) my1 = max(0, y2);
            if (y2 > my2) my2 = min(_pixelheight, y2);
        }
    }
    ...
    */
    ...
    uint32_t blend_screen(uint32_t dst, uint32_t src) {
        ...
        uint32_t rb_sub =
            ...
            (src & BLUE_MASK) * (f_b + 1))
            >>  8 & RB_MASK;
        uint32_t gn_sub = s_gn * (d_gn + 0x100)
            >> 16 & GN_MASK;
    ...
    uint32_t blend_overlay(uint32_t dst, uint32_t src) {
        ...
        uint32_t r = (d_r < 0x800000) ?
            d_r * ((s_r >> 16) + 1) >> 7 :
            0xFF0000 - ((0x100 - (s_r >> 16)) * (RED_MASK - d_r) >> 7);
        uint32_t g = (d_g < 0x8000) ?
            d_g * (s_g + 0x100) >> 15 :
    ...
    void Image::blit_resize(Image *img,
        ...
        srcX1 = max(0, srcX1);
        srcY1 = max(0, srcY1);
        srcX2 = min(img->_pixelwidth, srcX2);
        srcY2 = min(img->_pixelheight, srcY2);
        ...
        if(destW <= 0 || destH <= 0 || 
           ...
           srcX1 >= img->_pixelwidth || srcY1 >= img->_pixelheight) {
            return;
        }

        int64_t dx = (int)(srcW / (float)destW * PRECISIONF);
        int64_t dy = (int)(srcH / (float)destH * PRECISIONF);
